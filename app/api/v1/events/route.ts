  ...
  try {
    ...
    if (!user.discordId) {
      return NextResponse.json(
        ...
        { status: 403 } // forbidden, allowed but can't serve them cause we need disc id first.
      )
    }
    ...
    const quota = await db.quota.findUnique({
      // get quota, refreshes at the start of every month
      where: {
        userId: user.id,
        month: currentMonth,
        year: currentYear,
    ...
    if (quota && quota.count >= quotaLimit) {
      return NextResponse.json(
        ...
        { status: 429 } // payment required http status
      )
    }
    ...
    if (error instanceof z.ZodError) {
      return NextResponse.json(
        ...
        { status: 422 } // inappropriate content, is not valid
      )
    }
