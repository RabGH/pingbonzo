...
const Sidebar = ({ onClose }: { onClose?: () => void }) => {
  return (
    <div className="space-y-4 md:space-y-6 relative z-20 flex flex-col h-full">
      ...
      <Link
        href="/"
        className={cn("hidden sm:block text-lg/7 font-semibold text-brand-900")}
      >
        Ping<span className="text-brand-700">Panda</span>
      </Link>
      {/* Render plain text on smaller screens */}
      <p
        className={cn("block sm:hidden text-lg/7 font-semibold text-brand-900")}
      >
        Ping<span className="text-brand-700">Panda</span>
      </p>
